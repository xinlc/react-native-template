...
service.interceptors.request.use(
  async (config) => {
    ...
    if (token) {
      ...
      config.headers['X-Authorization'] = token;  // eslint-disable-line
    }
    return config;
  },
  (error) => {
  ...
  */
  ...
  (response) => {
    ...
    if (res.code !== 1 && res.code !== 20000) {
      // TODO:
      alert(res.msg || res.message || 'Error'); // eslint-disable-line
      ...
      if (res.code === 50008 || res.code === 50012 || res.code === 50014) {
        // to re-login
        alert('您已经注销，您可以取消以停留在此页面'); // eslint-disable-line
